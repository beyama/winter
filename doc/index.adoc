...
// get a provider for an optional Heater
val heaterProvider: () -> Heater? = graph.provider()

// get a set of instances of type Pump; this is useful when you have registered
// multiple Pumps with different qualifiers
val pumps: Set<Pump> = graph.instancesOfType<Pump>()

// get a set of providers for type Pump; this is useful when you have registered
// multiple Pumps with different qualifiers
...
----
class MyActivity : Activity() {
    // eager injection of a dependency
    private val api: GitHubApi by inject()
    // lazy injection of a dependency
