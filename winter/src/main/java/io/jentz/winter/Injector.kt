...
class Injector {
    ...
    class Instance<out T>(private val id: DependencyId) : AbstractEagerProperty<T>() {
        override fun getValue(graph: Graph): T {
            val provider = graph.retrieve(id) ?: throw EntryNotFoundException("Provider for ID `$id` not found.")
            @Suppress("UNCHECKED_CAST")
    ...
    class CurriedFactory<in A, out R>(private val id: DependencyId, private val argument: A) : AbstractEagerProperty<() -> R>() {
        override fun getValue(graph: Graph): () -> R {
            @Suppress("UNCHECKED_CAST")
            val provider = graph.retrieve(id) as? () -> (A) -> R ?: throw EntryNotFoundException("Provider for ID `$id` not found.")
    ...
    inline fun <reified T> instance(qualifier: String? = null, generics: Boolean = false) =
    ...
    inline fun <reified T> lazyInstance(qualifier: String? = null, generics: Boolean = false) =
